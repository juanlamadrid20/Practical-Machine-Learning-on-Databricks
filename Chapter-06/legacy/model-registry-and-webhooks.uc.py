...
# MAGIC %md
# MAGIC
# MAGIC ## MODELS IN UC Dont Support Webhooks.
# MAGIC
# MAGIC > https://docs.databricks.com/en/machine-learning/manage-model-lifecycle/index.html#limitations-on-unity-catalog-support
...
# MAGIC %md
# MAGIC > https://docs.databricks.com/en/mlflow/models-in-uc-example.html

# COMMAND ----------

# MAGIC %pip install --upgrade "mlflow-skinny[databricks]>=2.8.1" tensorflow
...
run_id = (
    mlflow.search_runs(experiment_id, order_by=["start_time DESC"])
    .head(1)["run_id"]
    .values[0]
)
...
catalog = "juan_dev"
schema = "mldbxbook"
model_name = f"{catalog}.{schema}.bank_customer_churn"
...
runs = mlflow.search_runs(experiment_details.experiment_id)
...
sorted_runs = (
    # mlflow.search_runs(experiment_details.experiment_id).sort_values("metrics.val_f1_score", ascending=False)
    mlflow.search_runs(experiment_details.experiment_id).sort_values(
...
model_uri = (
    f"runs:/{best_run_id}/{catalog}.{schema}.random_forest_classifier_featurestore"
)
...
try:
    ...
    print(
        f"Successfully registered model '{registry_model_name}' with URI '{model_uri}'."
    )
...
try:
    ...
    print(
        f"Successfully updated the description for the registered model '{model_details.name}'."
    )
except mlflow.exceptions.MlflowException as e:
...
try:
    ...
    print(
        f"Successfully updated the description for version {model_details.version} of the model '{model_details.name}'."
    )
except mlflow.exceptions.MlflowException as e:
    print(
        f"Failed to update version {model_details.version} of the model '{model_details.name}': {str(e)}"
    )
...
try:
    client.set_registered_model_alias(
        name=model_details.name, alias="Champion", version=model_details.version
    )
    print(
        f"Successfully transitioned version {model_details.version} of the model '{model_details.name}' to 'Champion'."
    )
except mlflow.exceptions.MlflowException as e:
    print(
        f"Failed to transition version {model_details.version} of the model '{model_details.name}' to 'Champion': {str(e)}"
    )
